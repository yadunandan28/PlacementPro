...
const runCode = async (language, code, stdin = "") => {
  ...
  const stdinNormalized = stdin ? (stdin.endsWith("\n") ? stdin : stdin + "\n") : "";
...
const judgeSubmission = async (language, code, testCases) => {
    ...
    try {
      ...
      if (result.compile && result.compile.code !== 0) {
        results.push({
          ...
          passed: false, runtime: elapsed, error: "COMPILE_ERROR",
        });
        continue;
      }
      ...
      if (result.run.code !== 0 || result.run.signal) {
        results.push({
          input: testCase.input, expectedOutput: testCase.expectedOutput,
          actualOutput: result.run.stderr || result.run.stdout || "Runtime Error",
          passed: false, runtime: elapsed, error: "RUNTIME_ERROR",
        });
        continue;
      }
      ...
      results.push({
        input: testCase.input, expectedOutput, actualOutput,
        passed: actualOutput === expectedOutput,
        runtime: elapsed, error: null,
      });
      ...
      results.push({
        ...
        passed: false, runtime: 0, error: "EXECUTION_ERROR",
      });
    }
  }
  ...
  return {
    results, testsPassed, testsTotal,
