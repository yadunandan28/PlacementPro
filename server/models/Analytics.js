...
const analyticsSchema = new mongoose.Schema(
  {
    user: {
      ...
      unique: true, // unique:true already creates an index — don't add schema.index({user:1})
    },
...
analyticsSchema.methods.recalculateOverall = function () {
  const scores = [this.dsaScore, this.dbmsScore, this.osScore, this.cnScore].filter(s => s > 0);
